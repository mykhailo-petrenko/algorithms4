 ...
 * in order to keep her minions on their toes.
 ...
 * For example, starting with n = 210022, k = 6, b = 3, t
 * he algorithm will reach the cycle of values [210111, 122221, 102212]
 * and it will stay in this cycle no matter how many times it continues iterating.
 * Starting with n = 1211,
 * the routine will reach the integer 6174,
...
public class Solution {
    ...
    public static String generate(String n, int b) {
        int k = n.length();

        char[] asc = n.toCharArray();
        Arrays.sort(asc);
        char[] desc = reverse(asc);

        String x = new String(desc);
        String y = new String(asc);
        ...
        char[] zeroes = new char[k - z.length()];
        Arrays.fill(zeroes, '0');

        return new String(zeroes) + z;
    }
