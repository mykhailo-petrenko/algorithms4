 ...
 *   1 <= k <= n
 */
public class MaximiseTheConfusionOnExam {

    public int maxConsecutiveAnswers(String answerKey, int k) {
        return Math.max(
            maxConsecutiveSymbol(answerKey, k, 'T'),
            maxConsecutiveSymbol(answerKey, k, 'F')
        );
    }

    private int maxConsecutiveSymbol(String answerKey, int k, char symbol) {
        int maxSymbol = 0;
        int replaced = 0;
        int max = 0;
        ...
        while (head < N) {
            if (answerKey.charAt(head) == symbol) {
                maxSymbol++;
            } else {
                replaced++;
            }
            ...
            while (replaced > k) {
                if (answerKey.charAt(tail++) == symbol) {
                    maxSymbol--;
                } else {
                    replaced--;
                }
            ...
            max = Math.max(max, maxSymbol + replaced);
            head++;
        }
