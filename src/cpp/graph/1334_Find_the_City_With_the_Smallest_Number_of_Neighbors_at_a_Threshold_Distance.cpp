...
class Solution {
    int count(int n, vector<vector<vector<int>>>& graph, int start, int distanceThreshold) {
        const int MAX = distanceThreshold + 1;
        vector<int> visited;
        visited.reserve(n);
        ...
        while (!q.empty()) {
            ...
            for (vector<int> neighbour: graph[node]) {
                q.push_back({neighbour[0], neighbour[1] + path});
            }
        ...
        int count = 0;
        for (int i=0; i<n; i++) {
            if (visited[i] <= distanceThreshold) {
                count++;
            }
        }
...
public:
    int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
        ...
        for (int i = n-1; i >=0; i--) {
            int neighbours = count(n, graph, i, distanceThreshold);
