/**
# LeetCode. 3068. Find the Maximum Sum of Node Values (https://leetcode.com/problems/find-the-maximum-sum-of-node-values/description/)
...
var maximumValueSum = function(nums, k, edges) {
  ...
  for (let i=0; i<N; i++) {
    const num = nums[i];
    const changed = nums[i]^k;

    max = max + num;

    const diff = changed - num;
  ...
  return Math.max(max + maxChange, max - minChange);
};
